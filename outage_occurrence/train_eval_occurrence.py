...
def main():
    ...
    # NEW: Remove 'year' before training so the model learns WEATHER, not DATES
    X_train_final = X_train.drop(columns=['year'], errors='ignore')
    X_test_final = X_test.drop(columns=['year'], errors='ignore')

    # ------------------------------------------------------------------
    # STEP 7: Train model
    # ------------------------------------------------------------------
    print("Training outage occurrence model (Generalization Mode)...")
    model = OutageOccurrenceModel()
    model.train(X_train_final, y_train) # Train on the version without 'year'
    ...
    preds, y_prob = model.predict(X_test_final)
