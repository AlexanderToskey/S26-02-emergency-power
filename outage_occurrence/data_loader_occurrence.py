...
def merge_occurrence_with_weather(
    ...
    # NEW: Defensive casting to ensure both dataframes speak the same language
    occurrence_df['date'] = pd.to_datetime(occurrence_df['date'])
    occurrence_df['fips_code'] = occurrence_df['fips_code'].astype(str)
    
    ghcnd_df['date'] = pd.to_datetime(ghcnd_df['date'])
    ghcnd_df['fips_code'] = ghcnd_df['fips_code'].astype(str)
...
def build_occurrence_labels(outages_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # 1. Automatically handle the unified column name
    cust_col = "customers_affected" if "customers_affected" in outages_df.columns else "customers_out"
    
    required_cols = ["fips_code", "run_start_time", cust_col]
    ...
    # 2. UNIVERSAL DATATYPES: Use .dt.normalize() to keep datetime64 format
    df["date"] = pd.to_datetime(df["run_start_time"]).dt.normalize()
    
    # 3. UNIVERSAL DATATYPES: Force fips_code to string
    df["fips_code"] = df["fips_code"].astype(str)

    grouped = (
        df.groupby(["fips_code", "date"])
        .agg(
            max_customers_affected=(cust_col, "max"),
        )
        .reset_index()
    )
...
def load_ghcnd_weather(file_path):
    ...
    # NEW: Force fips_code to string immediately upon loading
    if "fips_code" in ghcnd.columns:
        ghcnd["fips_code"] = ghcnd["fips_code"].astype(str)
