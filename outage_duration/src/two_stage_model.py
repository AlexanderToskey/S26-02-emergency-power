"""
two_stage_model.py - Two-stage outage duration prediction model

Stage 1: XGBClassifier predicts short (< 4hr) vs long (>= 4hr) outage.
...
then Stage 2 returns a duration in minutes.
"""
...
class TwoStageOutageModel:
    """
    Two-stage model for outage duration prediction.

    Stage 1: binary XGBClassifier (0=short, 1=long)
    Stage 2a: XGBRegressor trained on true-short samples
    Stage 2b: XGBRegressor trained on true-long samples

    At inference, Stage 1 routes, Stage 2 predicts duration in minutes.
    """
    ...
    def __init__(
        ...
        defaultShortParams = {
            ...
            'n_estimators': 500,
            'early_stopping_rounds': 50,
            'min_child_weight': 10,  # more conservative -- short outages are noisier
            'random_state': 42,
            'n_jobs': -1,
        }
        ...
        # ── Stage 2b: long outage regressor ──────────────────────────────────
        defaultLongParams = {
            'objective': 'reg:squarederror',
            'max_depth': 6,
            'learning_rate': 0.1,
    ...
    def train(
        ...
        Regressors train on log1p(minutes) and predict() returns expm1.
        Classifier threshold is swept on the validation set to maximize
        F1 for the long-outage class.
        """
        ...
        # drop invalid rows
        trainOk = np.isfinite(yTrainMin) & (yTrainMin >= 0)
        valOk = np.isfinite(yValMin) & (yValMin >= 0)

        xTrain = xTrain.loc[xTrain.index[trainOk]]
        yTrainMin = yTrainMin[trainOk]

        xVal = xVal.loc[xVal.index[valOk]]
        yValMin = yValMin[valOk]

        # binary labels: 0 = short, 1 = long
        yTrainLabel = (yTrainMin >= self.thresholdMin).astype(int)
        yValLabel = (yValMin >= self.thresholdMin).astype(int)
        ...
        print(
            ...
            f"long frac train={longFracTrain:.3f}, val={longFracVal:.3f}"
        )
        ...
        # balanced_acc = (recall_short + recall_long) / 2
        # this prevents the model from collapsing to always-short or always-long
        ...
        print(
            ...
            f"(val balanced_acc={bestBalAcc:.3f}, routing acc={routingAcc:.1f}%, F1 long={f1Long:.3f})"
        )

        # ── Stage 2a: short regressor (true-short subset only) ───────────────
        shortMaskTrain = yTrainMin < self.thresholdMin
        shortMaskVal = yValMin < self.thresholdMin

        xTrainShort = xTrain.loc[xTrain.index[shortMaskTrain]]
        yTrainShortLog = np.log1p(yTrainMin[shortMaskTrain])
        xValShort = xVal.loc[xVal.index[shortMaskVal]]
        yValShortLog = np.log1p(yValMin[shortMaskVal])

        print(
            f"\n[two_stage] Stage 2a short regressor: "
        ...
        # ── Stage 2b: long regressor (true-long subset only) ─────────────────
        longMaskTrain = yTrainMin >= self.thresholdMin
        longMaskVal = yValMin >= self.thresholdMin

        xTrainLong = xTrain.loc[xTrain.index[longMaskTrain]]
        yTrainLongLog = np.log1p(yTrainMin[longMaskTrain])
        xValLong = xVal.loc[xVal.index[longMaskVal]]
        yValLongLog = np.log1p(yValMin[longMaskVal])

        print(
            f"[two_stage] Stage 2b long regressor: "
    ...
    def predict(self, X: pd.DataFrame, softRouting: bool = False) -> np.ndarray:
        """
        Predict outage duration in minutes.

        softRouting=False (default): hard routing — each sample goes to exactly one
            regressor based on whether p_long >= classifierThreshold. Best overall
            accuracy because the ±30 min tolerance for short outages is too tight
            to allow any blending with the long regressor's 240+ min predictions.

        softRouting=True: probability-weighted blend of both regressors.
            pred = p_long * long_pred + (1-p_long) * short_pred
            Trades short accuracy (−14pp) for long accuracy (+17pp). Only useful
            if long outages are the primary concern.
        """
        ...
        if softRouting:
            ...
            shortPreds = np.expm1(np.clip(self.shortRegressor.predict(X), -20, 20))
            longPreds = np.expm1(np.clip(self.longRegressor.predict(X), -20, 20))
            return (1.0 - pLong) * shortPreds + pLong * longPreds
    ...
    def predictWithRouting(self, X: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        """
        Returns (predictions_in_minutes, routing_labels).

        routing_labels: 0 = routed to short regressor, 1 = routed to long regressor.
