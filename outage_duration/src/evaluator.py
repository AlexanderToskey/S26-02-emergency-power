...
def printTwoStageReport(
    ...
    print(f"  MAE:  {regressionMetrics['mae']:.2f} minutes")
    print(f"  RMSE: {regressionMetrics['rmse']:.2f} minutes")
    print(f"  MAPE: {regressionMetrics['mape']:.2f}%")

    print(f"\nTolerance Accuracy:")
    print(f"  Short outages (<4hr): {regressionMetrics['short_outage_accuracy']:.1f}% "
          f"(n={regressionMetrics['short_outage_count']})")
    print(f"  Long outages (>=4hr): {regressionMetrics['long_outage_accuracy']:.1f}% "
